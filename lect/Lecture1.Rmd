...
Monday, Wednesday 1:00-1:50 for synchronous lectures

Friday 1:00 or 2:00 (over zoom) for launch of asynchronous lab activities (i.e, Mini-Projects) 
]
...
office hours in Higgins 415/535 (by appointment) or over Zoom
]
...
```{r,echo=FALSE}
g <- rbind(c("","","Points"),
  c("Class participation and engagement","",50),
...
- Interrupt me!
- No exams, no quizzes
- Engagement contributes 10% to grade!
...
- Meet over Zoom at 1:00 or 2:00 (synchronous)
...
- MP descriptions release before Friday meetings
- 2-3 weeks to finish MP
- Reports due on Wednesdays
- Team-based
- Report written in R Markdown (MP1)
...
- Additional presentation (10 min)
- Record presentation and deliver to class
...
- ~1 month to develop, execute
- Start mid April
- Presentation due 1 week before report (May 5)
- Report due on scheduled final date (May 12)
...
res%>%pivot_longer(cols=c("physics","scripting"),values_to = "resp")%>%
  ggplot(aes(name,resp))+geom_boxplot()+ggtitle("How comportable are you with . . . ")+theme_classic(20)+xlab("")
...
knitr::include_graphics("https://useruploads.socratic.org/cEoiMUubQWGFHjA0wFhd_Torque.jpg")
```
]
...
.pull-left[Is this a reasonable prediction? 1600 N?! A typical bullet is ~500N

Let's first see how the max changes with merus length (L), a standard measurement for the group. 
...
$$F(m)=a+Lb$$
with coefficents from Blanco and Patek (2014) where $a=-4.4$ and $2.1$. So the functions takes the form
...
Now, establish a length range (8-16 cm) and predict merus length?
...
```{r,dev='svg',fig.show="hold", out.width="30%",out.height="10%",fig.height=6}
...
```{r,dev='svg',fig.show="hold", out.width="50%",out.height="20%",fig.height=6}
qplot(L,merus.pred)+theme_classic(30)+ylab("Force (N)")+geom_point(aes(x=15,y=1600),col="red")
knitr::include_graphics("mantis.png")

...
## Conceptual basis
In lecture recenlty we explored the concepts of stress ($\sigma$), strain ($\varepsilon$), stiffness ($E$, Young's modulus) and strength ($\sigma_{max}$). These are all material properties and represent mechanical means to characterize biomaterials. These physical quantities are summarized here:
...
where d is the deflection, $F$ is the applied force, l is the length of the beam, and $EI$ is the flexural stiffness. Using \autoref{eq:stress}, if we measure the deflection of a beam of length $l$ under a known load F, we can derive an estimate for the flexural stiffness of the beam, $EI$.
...
### Preparing your sample
For this part of the lab, you will be three-point bending a biomaterial (wood . . . birch, methinks). You will apply an increasing load to piece of wood spanning a know distance and record the deformation. Break a piece in half. Wet one of the broken halfs materials under water for 5 minutes.

#### Measuring load and deformation
For each half, place the specimen in the three point tester under the load cell. Prof. Kenaley will help you initiate the program that reads the voltage data from the load cell system. The program will apply depress a cylinder against the span of the wood beam while measuring the distance and load. We will impose 5 cycles of depression and retraction (load and unloading) at two speeds, fast and slow.

Repeat the procedure, but now flip both the dry and wet pieces 90$^\textrm{o}$ to load the deep dimension on the thin face.
...
kable(mat.data, caption = "Material Data")
```
...
kable(mat.data, caption = "Intestine Data")
...
1. What evidence do you have that intestinal lining is anisotropic? 
2. What do you think the function of anisotropy is in the function of an intestine? 
3. In your beam analysis, did you find any evidence of viscoelasticity?
4. Did beam orientation result in different deformation and different flexural stiffness? 

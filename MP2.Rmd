...
You task is to render all this in repeatable and adjustable computational model as an R function. According to the model, this function should be named `model` and take as arguments the following:
  ...
  + `Lo`: Length of the inlever, $L_i$ in m.
...
model <- function(mass,Lmuscler,Lmuscle,Li,rho=0.00105,k=80,theta,opt=0.8){
 ...
 fmax <- PCSA*k
 ffl <-  function(Lmuscle,Lmuscler,opt=0.5){-6.25*(Lmuscle/(Lmuscler*opt))^2+12.5*(Lmuscle/(Lmuscler*opt))-5.25}
...
Reports are due by Wednesday, February 14st at 11:59 pm. You should compress your directory containing the .Rmd, all class data, and videos of each member assessing the model. This .zip file should be uploaded [here](https://forms.gle/JPcZLjrrB7UKLhcj7).
